...
if __name__ == "__main__":
    Utils.init_logging("Kirby's Epic Yarn Client", exception_logger="Client")
...
class CommonContext:
    ...
    def make_gui(self) -> "type[kvui.GameManager]":
        ...
        class TextManager(GameManager):
            base_title = "Kirby's Epic Yarn Client"
...
def main(*args):
    class KEYContext(CommonContext):
        ...
        async def server_auth(self, password_requested: bool = False):
            if password_requested and not self.password:
                await super(KEYContext, self).server_auth(password_requested)
            await self.get_username()
            await self.send_connect(game="Kirby's Epic Yarn")
    ...
    async def _main(args):
        ctx = KEYContext(args.connect, args.password)
        ctx.auth = args.name
        ctx.server_task = asyncio.create_task(server_loop(ctx), name="server loop")
...
if __name__ == '__main__':
    ...
    main(*sys.argv[1:])  # default value for parse_args